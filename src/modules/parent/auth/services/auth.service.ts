...
// // src/modules/parent/services/auth.service.ts
...
import { CACHE_MANAGER } from '@nestjs/cache-manager';
import { Cache } from 'cache-manager';
import { firebaseAuth } from 'src/config/firebase.config'; // This is Firebase Admin SDK
...
export class ParentAuthService {
  ...
  async loginWithPhoneNumber(idToken: string) {
    try {
      // Backend uses firebaseAuth (Admin SDK) to verify the ID token
      const decodedToken = await firebaseAuth.verifyIdToken(idToken);
      const firebaseUser = await firebaseAuth.getUser(decodedToken.uid);
