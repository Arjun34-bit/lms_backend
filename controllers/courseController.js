...
res.status(200).json({ received: req.body });
    ...
    if (!title || !description || !category_id || !department_id || !subject_id || !level) {
      return res.status(400).json({ message: "Missing required fields: title, description, category, department, subject, or level." });
    }
    ...
    if (!courseData.instructor_id) {
      return res.status(403).json({ message: "Unauthorized: Instructor ID is required." });
    }
    ...
    const logMessage = `[${new Date().toISOString()}] Error creating course: ${err.message}\n${err.stack}\n\n`;
