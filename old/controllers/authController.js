...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .lean()
      .select(
        'email name role phone_number status departments subjects address googleId facebookId appleId createdAt',
      );
    if (!user) {
    ...
    const departmentIds =
      user?.departments ??
      user.departments.split(',').map((id) => new mongoose.Types.ObjectId(id));
    const subjectIds =
      user?.subjects ??
      user.subjects.split(',').map((id) => new mongoose.Types.ObjectId(id));

    let departmentDetails, subjectDetails;
    ...
    if (departmentIds) {
      ...
      departmentDetails = departments.map((department) => ({
        id: department._id,
...
exports.validateToken = async (req, res) => {
  try {
    ...
    if (!authHeader) {
      return res
        .status(401)
        .json({ valid: false, message: 'Authorization header is missing.' });
    }

    if (!authHeader.startsWith('Bearer ')) {
      return res.status(401).json({
        valid: false,
        message: 'Token is missing or invalid. It must start with "Bearer ". ',
      });
    }
    ...
    if (!storedToken) {
      return res
        .status(401)
        .json({ valid: false, message: 'Token not found in the database.' });
    }
    ...
    if (err instanceof jwt.JsonWebTokenError) {
      return res
        .status(401)
        .json({ valid: false, message: 'Invalid token.', error: err.message });
    }
...
exports.register = async (req, res) => {
  try {
    const { email, password, role, phone_number, name, departments, subjects } =
      req.body;
...
exports.linkSocialAccount = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error linking social account', error: error.message });
  }
};
